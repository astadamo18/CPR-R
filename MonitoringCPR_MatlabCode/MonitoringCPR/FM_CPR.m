...
if isequal(band,'And91')
    bandw = And_HAC91([u_ols(2:end,1) v],kern);
elseif isequal(band,'NW')
    bandw = bwNW([u_ols(2:end,1) v],kern,0,[]);
else
    bandw = band;
end
if isequal(band,'AM92')
    [Lr,Dr,~] = AndMon_HAC92([u_ols(2:end,1) v],kern,1,1,deme);
else
    [Lr,Dr,~] = lr_var([u_ols(2:end,1) v],kern, bandw, deme);
end
...
Astar = zeros(size(Z_,2),1); % Astar is a Vector
if ~isempty(w) % This is the case, when stationary regressors are included
    Astar(1:kw,:) = (1/T)*w(2:end,:)'*u_ols(2:end,1) - (1/T)*w(2:end,:)'*v*Lr_vvvu;
end
Astar(end-P(end)+1:end,:) = Mstar;  
result.Astar = Astar;
result.Lambda0 = Lambda0;
% (3) FM Estimator
yplus = y(2:end,:)-v*Lr_vvvu;
bplus = iZZ*(Z(2:end,:)'*yplus-Astar); % FM Estimator for CPR.
...
if ~isempty(w)
    ...
    S = w_.*(result.u_plus*ones(1,size(w,2)));
    if isequal(band,'AM92')
